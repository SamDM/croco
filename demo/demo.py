...
def main():
    ...
    image1 = (
        trfs(Image.open(this_dpath / "../assets/Chateau1.png").convert("RGB"))
        .to(device, non_blocking=True)
        .unsqueeze(0)
    )
    image2 = (
        trfs(Image.open(this_dpath / "../assets/Chateau2.png").convert("RGB"))
        .to(device, non_blocking=True)
        .unsqueeze(0)
    )

    # load model
    ckpt = torch.load(this_dpath / "../checkpoints/CroCo_V2_ViTLarge_BaseDecoder.pth", "cpu")
    ...
    fname = this_dpath / "outputs/demo_output.png"
    visualization.save(fname)
    print("Visualization save in " + str(fname))
